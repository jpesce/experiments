...
function drawLine(gridSize, brightness, x, y) {
  ...
  if (!CURRENT_OPTIONS.invert) {
    weight =
      Math.round(map(255 - brightness, 0, 255, 1, weightQuantity)) *
      (gridSize / 2 / weightQuantity);
    angle =
      Math.round(map(255 - brightness, 0, 255, 0, anglesQuantity)) *
      (90 / anglesQuantity);
    fill(0);
  }
  ...
  if (CURRENT_OPTIONS.invert) {
    weight =
      Math.round(map(brightness, 0, 255, 1, weightQuantity)) *
      (gridSize / 2 / weightQuantity);
    angle =
      Math.round(map(brightness, 0, 255, 0, anglesQuantity)) *
      (90 / anglesQuantity);
    fill(255);
  }
...
function drawSquare(gridSize, brightness, x, y) {
  ...
  const blackOrWhite = brightness > treshold ? 0 : 255;
  noStroke();
  colorMode(HSB, 255);
...
function drawText(gridSize, brightness, x, y) {
  ...
  if (!CURRENT_OPTIONS.invert) {
    index = Math.round(
      map(255 - brightness, 0, 255, 0, characterMap.length - 1),
    );
    fill(0);
  }
